...
// 绘制行内输入框
void draw_input_line_box(const struct Position pos, const int width) {
...
// 绘制表单
void draw_forms(const struct Position form_pos, const char *form_title,
                const struct Item items[16], const int item_count) {
    struct Size form_size;
    struct Position input_pos;
    form_size.width = 34;
    form_size.height = item_count * 2 + 6;
    draw_frame(form_pos, form_size, form_title, "");
    for (int i = 0; i < item_count; ++i) {
        move_cursor(form_pos.line + 2 * (i + 1), form_pos.col + 2);
        printf("%s", items[i].title);
        input_pos.line = form_pos.line + 2 * (i + 1);
        input_pos.col = form_pos.col + 14;
        draw_input_line_box(input_pos, 18);
    }
    input_pos.line += 2;
    input_pos.col -= 3;
    struct Button ok_btn = {1, "　　完成"};
    struct Size size_btn = {14, 3};
    draw_button(input_pos, ok_btn, size_btn);
}
